...
;(function($) {
	...
	/*
	 * Reveal the hover text for the plants/floral section
	 * @param{Object} self The element whose text is to be revealed
	 * @param{String} category The type of element (plant or floral)
	 */
	function addPlantFloralHover(self, category) {
		if (!self.hasClass("plant_floral-reveal-wrapper")) {
			self.addClass("plant_floral-reveal-wrapper").attr("data-touched", true);
			$(".plant_floral-ampersand").addClass("plant_floral-" + category + "-ampersand");
		}
	};
	
	/*
	 * Removes the hover effect from the plants/floral section
	 * @param{Object} self The element to effect the change upon
	 * @param{String} category The type of element (plant or floral)
	 */
	function removePlantFloralHover(self, category) {
		if (self.hasClass("plant_floral-reveal-wrapper")) {
			self.removeClass("plant_floral-reveal-wrapper").attr("data-touched", false);
			$(".plant_floral-ampersand").removeClass("plant_floral-" + category + "-ampersand");
		}
	};
	
	$(document).ready(function() {
		// set up plant and floarl hover effects for mouse and mobile (touch) devices
		$("#plant_floral-wrapper").on("mouseover.hoverPlant touchend.hoverPlant", ".plant_floral-reveal-wrapper-left[data-touched='false']", function(evt) {
			if (evt.preventDefault) {
				evt.preventDefault();
			}
			addPlantFloralHover($(".plant_floral-reveal-wrapper-left"), "plant");
		}).on("mouseout.outPlant", "#plant-wrapper", function() {
			removePlantFloralHover($(".plant_floral-reveal-wrapper-left"), "plant");
		}).on("touchend.outPlant", ".plant_floral-reveal-wrapper-left[data-touched='true']", function(evt) {
			if (evt.preventDefault) {
				evt.preventDefault();
			}
			removePlantFloralHover($(".plant_floral-reveal-wrapper-left"), "plant");
		}).on("mouseover.hoverFloral touchend.hoverFloral", ".plant_floral-reveal-wrapper-right[data-touched='false']", function(evt) {
			if (evt.preventDefault) {
				evt.preventDefault();
			}
			addPlantFloralHover($(".plant_floral-reveal-wrapper-right"), "floral");
		}).on("mouseout.outFloral", "#floral-wrapper", function() {
			removePlantFloralHover($(".plant_floral-reveal-wrapper-right"), "floral");
		}).on("touchend.outFloral", ".plant_floral-reveal-wrapper-right[data-touched='true']", function(evt) {
			if (evt.preventDefault) {
				evt.preventDefault();
			}
			removePlantFloralHover($(".plant_floral-reveal-wrapper-right"), "floral");
